...
import { Bot, Send, Loader2, Sparkles, Zap, FileText, AlertTriangle, ChevronDown } from 'lucide-react'
...
function MarkdownText({ text }: { text: string }) {
  ...
  const rendered = parts.map((part, i) => {
    if (part.startsWith('```') && part.endsWith('```')) {
      ...
      return (
        <pre key={i} className="my-2 bg-gw-dark rounded p-3 text-xs font-mono text-gw-green overflow-x-auto whitespace-pre-wrap">
          {code}
        </pre>
      )
    }
  ...
  return (
    <div className={`flex gap-3 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
      ...
      <div className={`max-w-[80%] rounded-xl px-4 py-3 ${
        isUser
          ? 'bg-gw-green/10 border border-gw-green/20 text-sm text-white'
          : 'bg-gw-panel border border-gw-border'
      }`}>
        {isUser
      ...
      <div className="flex-1 flex flex-col max-w-4xl w-full mx-auto px-4 pb-4">
        ...
        <div className="py-5 border-b border-gw-border flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gw-muted">AI assistant with live WORM ledger access · Tenant: {tenantId}</p>
          </div>
          <div className="ml-auto flex items-center gap-1.5 text-xs text-gw-green">
        ...
        <div className="flex-1 overflow-y-auto py-6 space-y-5 min-h-0" style={{ maxHeight: 'calc(100vh - 280px)' }}>
            ...
            <div className="flex flex-col items-center justify-center py-12 gap-8">
              <div className="text-center">
                ...
                <p className="text-sm text-gw-muted max-w-sm">
                  I have access to your live WORM ledger, grid carbon intensity, power telemetry, incidents, and Scope 1 emissions.
                </p>
              </div>
          ...
          {error && (
            <div className="flex gap-3">
              ...
              <div className="bg-red-900/20 border border-red-500/30 rounded-xl px-4 py-2.5 text-sm text-red-400">
                {error}
              </div>
            </div>
          )}
        ...
        <div className="border-t border-gw-border pt-4">
          {!isEmpty && (
            <button
              onClick={() => setMessages([])}
              className="mb-2 text-xs text-gw-muted hover:text-white transition-colors"
            >
              Clear conversation
            </button>
          )}
